...
class Api_tipos_productos:
    def get(self, id_producto, nombre_producto):
        try:
            # http://0.0.0.0:8080/api_tipos_productos?user_hash=12345&action=get
            if id_producto is None and nombre_producto is None:
            ...
            elif id_producto is not None and nombre_producto is None:
                ...
                web.header('Content-Type', 'application/json')
                return json.dumps(tipos_productos_json)
            elif id_producto is None and nombre_producto is not None:
                # api_tipos_productos?user_hash=dc243fdf1a24cbced74db81708b30788&action=get&nombre_producto=Nombre
                result = config.model.get_producto(str(nombre_producto))
                tipos_productos_json = []
                tipos_productos_json.append(dict(result))
                web.header('Content-Type', 'application/json')
                return json.dumps(tipos_productos_json)
        ...
        try:
            ...
            if user_hash == 'dc243fdf1a24cbced74db81708b30788':
                if action is None:
                    ...
                    return self.get(id_producto, nombre_producto)
                elif action == 'put':
